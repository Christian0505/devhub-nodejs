...
app.put('/addUser', async (req, res) => {
  ...
  app.post('/crea-utente', (req, res) => {
  const { username, password, email } = req.body;
  // Verifica che i campi siano presenti
  if (!username || !password || !email) {
    return res.status(400).json({ errore: 'username, password ed email sono obbligatori' });
  }
...
app.listen(PORT, () => {
  console.log(`Server avviato sulla porta ${PORT}`);
    ...
    try {
    // Controlla username
    const usernamePresente = await User.findOne({ username });
    if (usernamePresente) {
      return res.status(409).json({ errore: 'Username già in uso' });
    }

    // Controlla email
    const emailPresente = await User.findOne({ email });
    if (emailPresente) {
      return res.status(409).json({ errore: 'Email già registrata' });
    }
    // Nessun conflitto: crea e salva il nuovo utente
...
app.post('/movies', (req, res) => {
  // Estrae i parametri dal body 
  const { title, director, year } = req.body;

  // Verifica che tutti i parametri obbligatori siano presenti
  if (!title || !director || !year) {
    return res.status(400).json({
      error: 'I parametri "title", "director" e "year" sono obbligatori e devono essere valorizzati.'
    });
  }
});
     // apro la connessione a mongodb
...
app.get('/listMovies', async (req, res) => {
    ...
    app.post('/film/filtra', async (req, res) => {
